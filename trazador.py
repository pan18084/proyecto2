...
# Selección de los primeros 3, los 3 puntos del medio y los últimos 3 para el ajuste
primeros_dias = dias[:3]
primeros_precios = precios[:3]
puntos_medios_dias = dias[6:9]
puntos_medios_precios = precios[6:9]
ultimos_dias = dias[-3:]
ultimos_precios = precios[-3:]

# Combinar los puntos seleccionados
dias_ajuste = np.concatenate((primeros_dias, puntos_medios_dias, ultimos_dias))
precios_ajuste = np.concatenate((primeros_precios, puntos_medios_precios, ultimos_precios))
...
plt.scatter(dias_ajuste, precios_ajuste, label=f'3 Primeros, 3 del Medio y 3 Últimos Datos (Usados en Ajuste)', color='blue')
plt.plot(dias_evaluacion, precios_evaluacion, label='Trazador Cúbico Proyectado', color='red')
plt.xlabel('Día')
plt.ylabel('Precio de Bitcoin')
plt.title(f'Trazador Cúbico de Bitcoin (Ajuste con 3 Primeros, 3 del Medio y 3 Últimos Puntos)')
plt.legend()
plt.grid(True)
plt.show()
